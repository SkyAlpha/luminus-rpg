...
export class LuminusAnimationManager extends AnimationNames {
	...
	isMoving() {
		...
		return this.entity.container.body.velocity.x !== 0 || this.entity.container.body.velocity.y !== 0;
	}
	...
	animateWithAngle(animation, angle) {
		...
		if (parseFloat(angle).toFixed(2) > -0.66 && parseFloat(angle).toFixed(2) < 0.66) {
			this.entity.anims.play(animation + this.rightAnimationSufix, true);
			this.entity.flipX = false;
		} else if (parseFloat(angle).toFixed(2) > -2.33 && parseFloat(angle).toFixed(2) < -0.66) {
			this.entity.anims.play(animation + this.upAnimationSufix, true);
		} else if (
			(parseFloat(angle).toFixed(2) < -2.33 && parseFloat(angle).toFixed(2) >= -3.14) ||
			(parseFloat(angle).toFixed(2) <= 3.14 && parseFloat(angle).toFixed(2) > 2.33)
		) {
			if (this.entity.anims.animationManager.exists(animation + this.leftAnimationSufix, true)) {
